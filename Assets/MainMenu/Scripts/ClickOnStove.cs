...
public class ClickOnStove : MonoBehaviour
{
    ...
    {
        if(Stats.ActiveItemName == "kettleWithBeans")
        {
        ...
        if (Stats.ActiveItemName == "Bottle")
        {
            var fields = dialogWindow.GetComponentsInChildren<TMP_Text>();
            fields[0].text = names[1];
            fields[1].text = replics[1];
            hideOnLB = true;
            cd = cooldown;
            Activate();
            ...
            for (int i = 0; i < cellContainer.transform.childCount; i++)
            {
                ...
                if (cell.name.Contains("Cell"))
                {
                    ...
                    cellContainer.transform.GetChild(i).name = "bottleWithBeans";
